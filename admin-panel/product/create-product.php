        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (!in_array($mainImage['type'], $allowedTypes)) {
                    ...
                    $uploadPath = '../../images/' . $categoryName . '/' . $mainImageName;
                    
                    // Create directory if it doesn't exist
                    if (!file_exists('../../images/' . $categoryName)) {
                        mkdir('../../images/' . $categoryName, 0755, true);
                    }
                    
                    // Move uploaded file
                    if (move_uploaded_file($mainImage['tmp_name'], $uploadPath)) {
                        // Insert product into database
                        $stmt = $conn->prepare("INSERT INTO product (categoryID, name, shortDesc, stock, price, discount, mainImage, productDetail) 
                                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                        $stmt->execute([$categoryID, $name, $shortDesc, $stock, $price, $discount, $mainImageName, $productDetail]);
                        $productID = $conn->lastInsertId();
                        
                        // Process sub images if any
                        if (!empty($_FILES['subImages']['name'][0])) {
                            $subImageCount = 1; // Start counting from 2 (-2, -3, etc.)
                            ...
                            foreach ($_FILES['subImages']['tmp_name'] as $key => $tmpName) {
                                if ($_FILES['subImages']['error'][$key] === UPLOAD_ERR_OK) {
                                    ...
                                    $subImageName = strtolower(str_replace(' ', '-', $name)) . '-' . ($subImageCount + 1) . '.' . $subExtension;
                                    $subUploadPath = '../../images/' . $categoryName . '/' . $subImageName;
                                    
                                    if (move_uploaded_file($tmpName, $subUploadPath)) {
                                        // Insert sub image into database
...
<body>
        ...
        <div class="product-create-form-wrapper">
            ...
            <form action="" id="productForm" method="POST" enctype="multipart/form-data" class="product-create-form">
                ...
                <div class="form-group">
                    ...
                    <textarea id="shortdesc" name="shortdesc" rows="3" placeholder="Enter short description of product"></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="discount" name="discount" value="0" placeholder="Enter product discount" step="0.1" required />
                </div>
                ...
                <div class="form-group">
                    ...
                    <!-- Default text or uploaded image will appear here -->
                        <img src="" alt="Main Image" id="main-image" />
                    </div>
                    <div id="no-image-message" style="display: block; color: #777;">Please upload an image</div> <!-- Default text -->
                        <input type="file" id="main-image-upload" name="mainImage" style="display: none;" accept="image/*" onchange="uploadMainImage(event)" />
                    
                    <div class="form-group-buttons">
                        <button type="button" id="upload-main-image" class="btn-upload" onclick="triggerMainImageInput()">Upload</button>
                    </div>
                </div>    

                <div class="form-group">
                    <label for="productdetail">Product Detail <span>*</span><span class="label-tip"> (Please use ' | ' to separate each detail)</span></label>
                    <textarea id="productdetail" name="productdetail" rows="6" placeholder="Enter full product detail here"></textarea>
                </div>

                <div class="form-group">
                    <label>Sub Images <span class="label-tip">(Optional)</span></label>
                ...
                <button type="button" id="submitBtn" class="btn-create-submit">Create Product</button>
            </form>
        </div>
    </main>
    ...
    <div class="confirm-modal" id="confirmModal">
        <div class="confirm-content">
            <div class="confirm-message" id="confirmMessage">Are you sure you want to create this product?</div>
...
<script>
    // Main image handling (keep your existing code)
    ...
    // Sub-image handling - completely new implementation
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Form submission handling (keep your existing code with small modification)
